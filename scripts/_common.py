#!/usr/bin/env python3
"""
md-pay-hub/scripts/_common.py
Shared utilities for all Maryland Pay Hub search scripts.
Maryland Wage Range Transparency Act (HB649) — effective Oct 1, 2024 — employers with 15+ employees must include wage range + benefits
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"md-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/md-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "MD"
...
REGION_TERMS = ['maryland', 'baltimore', 'bethesda', 'rockville', 'silver spring', 'college park', 'annapolis', 'gaithersburg', 'columbia, md', 'laurel, md', 'bowie', 'greenbelt', 'hyattsville', 'chevy chase', 'potomac', 'germantown', 'frederick, md', 'hagerstown', 'salisbury, md', 'bel air, md', ', md', 'md,']

_NON_REGION_LOC_TERMS = ['toronto', 'vancouver', 'ontario, canada', 'british columbia', 'canada,', 'washington, dc', 'washington dc', 'district of columbia', 'denver, co', 'boulder, co', 'seattle, wa', 'washington state', 'new york city', 'new york, ny', 'san francisco, ca', 'los angeles, ca', 'chicago, il', 'boston, ma', 'minneapolis, mn', 'newark, nj', 'jersey city, nj']
...
EXTRACT_PROMPT = """\
Extract ONE Maryland job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Maryland job with explicit USD salary range is found:
{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Maryland location","source_url":"{url}","posted":"YYYY-MM-DD"}
