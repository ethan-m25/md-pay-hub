#!/usr/bin/env python3
"""
md-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Maryland edition.
...
Run: python3 ~/md-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/md-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/md-pay-hub/scripts/.greenhouse.lock")
...
SEED_SLUGS = [
    ("thebaltimorebanner", "The Baltimore Banner"),
    ("leidos", None),
...
def is_md_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_md_job(title, location, content_text):
            continue
...
def main():
    ...
    log("=== MD Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new MD jobs written to {OUTPUT_FILE} ===")
