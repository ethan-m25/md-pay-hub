#!/usr/bin/env python3
"""
md-pay-hub/scripts/search-lever.py
Lever job board scraper — Maryland edition.
...
Run: python3 ~/md-pay-hub/scripts/search-lever.py
"""
...
LOG_FILE  = os.path.expanduser("~/md-pay-hub/scripts/lever.log")
LOCK_FILE = os.path.expanduser("~/md-pay-hub/scripts/.lever.lock")
...
SEED_SLUGS = load_lever_seeds('md')
    "alertus",  # auto-discovered 2026-05-28 — 18 MD+salary
    "lendbuzz",  # auto-discovered 2026-05-28 — 8 MD+salary
SEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))


DISCOVERY_QUERIES = [
    'site:jobs.lever.co "Maryland" OR "Baltimore" OR "Bethesda" OR "Rockville" salary range 2025 2026',
    'site:jobs.lever.co "Maryland" OR "Baltimore" OR "Bethesda" OR "Rockville" engineer OR analyst OR manager salary',
    'site:jobs.lever.co "Maryland" OR "Baltimore" OR "Bethesda" OR "Rockville" "pay range" 2025 2026',
]
...
def is_md_location(location_str: str, desc_text: str = "") -> bool:
...
def main():
    ...
    for slug in all_slugs:
        ...
        for job in jobs:
            ...
            if not is_md_location(loc_name, desc_plain):
                continue
            region_count += 1
        ...
        log(f"  MD: {region_count} | New w/ salary: {found_this}")
    ...
    if newly_qualified:
        ...
        try:
            ...
            for slug, count in sorted(newly_qualified.items(), key=lambda x: -x[1]):
                if f'"{slug}"' in source:
                    continue
                log(f"  + {slug} ({count} MD+salary jobs)")
                new_lines.append(f'    "{slug}",  # auto-discovered {TODAY} — {count} MD+salary')
            if new_lines:
                insert_block = "\n".join(new_lines)
                marker = "\nSEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))"
                if marker in source:
