#!/usr/bin/env python3
"""
md-pay-hub/scripts/search-workday.py
Workday API scraper — Maryland edition.

MD Wage Range Transparency Act (effective Oct 1, 2024)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Maryland employers
  2. CXS JSON API   — paginate all jobs per employer, filter Maryland (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/md-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/md-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/md-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Maryland"
...
SEED_TENANTS = [
    ...
    ("troweprice.wd5.myworkdayjobs.com", "troweprice", "External", "T. Rowe Price"),
    ("underarmour.wd5.myworkdayjobs.com", "underarmour", "External", "Under Armour"),
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Maryland" salary 2026',
    'site:myworkdayjobs.com "Baltimore" salary "$" 2026',
    'site:myworkdayjobs.com "Bethesda" OR "Rockville" OR "Gaithersburg" salary 2026',
    'site:myworkdayjobs.com Maryland defense OR government OR healthcare salary 2026',
    'site:myworkdayjobs.com "Columbia" OR "Silver Spring" OR "Frederick" MD salary 2026',
]
...
SITEMAP_SOURCES = [
    ("https://bah.wd1.myworkdayjobs.com/en-US/BAH_Jobs/siteMap.xml", "Booz Allen Hamilton"),
]
...
def is_maryland(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Maryland.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_maryland(p.get("locationsText", ""), p.get("externalPath", "")):
                    md_jobs.append(p)
            offset += limit
        ...
        log(f"  Maryland jobs: {len(md_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(md_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_maryland(location, external_path):
            continue
